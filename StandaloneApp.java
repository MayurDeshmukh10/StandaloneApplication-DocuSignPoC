...
public class StandaloneApp {
	...
	public static void main(String[] args) throws Exception
	{
		...
		try{
			http.authenticate();
			http.sendGroupSigning();			//For Group Signing 
			//http.sendPost();					//For individual signing
		}
		catch(Exception e){
	...
	private String displayHTTPResponseGroupSigning(HttpURLConnection con) throws Exception{
	...
	private String createEnvelopeBodyContentGroupSigning(String docBase64json,String document_type,String groupId){
	...
	private void sendGroupSigning() throws Exception{

		java.io.Console console = System.console();
		System.out.println();
		String doc_name = console.readLine("Enter Document Name : ");
		String doc_type = console.readLine("Enter Document Type/Extension : ");
		String groupId = console.readLine("Enter GroupId : ");
		Base64.Encoder encoder = Base64.getEncoder(); ;
		byte[] buffer = readFile(doc_name);
		String docBase64 = new String(encoder.encodeToString(buffer));
		String url = "https://demo.docusign.net/restapi/v2/accounts/"+accountid+"/envelopes";
		...
		con.setRequestMethod("POST");		//HTTP request Header
		setRequestHeader(con);	
		String jsonInputString = createEnvelopeBodyContentGroupSigning(docBase64,doc_type,groupId);		//HTTP request data in JSON
		con.setDoOutput(true);											//For writing data using OutputStream of HTTP
		try(OutputStream os = con.getOutputStream()) 
		{
			byte[] input = jsonInputString.getBytes("utf-8");
		...
		displayHTTPResponseGroupSigning(con);				//For Reading HTTP response
	}
